...
CREATE TABLE IF NOT EXISTS historico_medico_pet (
    ...
    FOREIGN KEY (Id_pet) REFERENCES pet(Id_pet) ON DELETE CASCADE,
    CHECK (Tipo IN (
        'VACINA', 'REMEDIO', 'DOENCA', 'TRATAMENTO',
        'RESULTADO_EXAME', 'HISTORICO', 'RESTRICAO_MOBILIDADE',
        'COMPORTAMENTO', 'ALIMENTACAO', 'OUTROS'
    ))
);

CREATE TABLE IF NOT EXISTS solicitacao_adocao (
    Id_solicitacao_adocao BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    Id_pet BIGINT NOT NULL,
    Id_protetor BIGINT NOT NULL,
    Id_adotante BIGINT NOT NULL,
    Situacao VARCHAR(20) NOT NULL,
    CHECK (Situacao IN ('Aceita', 'Recusada', 'Pendente')),
...
CREATE TABLE IF NOT EXISTS adocao (
    ...
    CHECK (Status IN ('Adotado', 'Disponivel', 'Disponível', 'Indisponivel', 'Indisponível')),
    FOREIGN KEY (Id_pet) REFERENCES pet(Id_pet),
    FOREIGN KEY (Id_adotante) REFERENCES usuario(Id_usuario),
    FOREIGN KEY (Id_protetor) REFERENCES usuario(Id_usuario)
);
...
CREATE TABLE IF NOT EXISTS historico_adocao (
    ...
    FOREIGN KEY (Id_protetor) REFERENCES usuario(Id_usuario) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS perfil_match (
    Id_perfil_match BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    Id_usuario BIGINT NOT NULL UNIQUE,
    gato BOOLEAN DEFAULT false,
    cachorro BOOLEAN DEFAULT false,
    macho BOOLEAN DEFAULT false,
    femea BOOLEAN DEFAULT false,
    pequeno BOOLEAN DEFAULT false,
    medio BOOLEAN DEFAULT false,
    grande BOOLEAN DEFAULT false,
    convive_bem BOOLEAN DEFAULT false,
