...
useEffect(() => {
  const carregarDados = async () => {
    ...
    try {
      let perfil = null;
      try {
        perfil = await buscarPerfilMatch(idUsuario);
        setPerfilMatch(perfil);
        setPodeOrdenar(!!perfil);
      } catch (error) {
      ...
      if (perfil) {
        const pontuacoes = await buscarPontuacaoMatch(pets, perfil);
        console.log("Scores associados: ",pontuacoes)
