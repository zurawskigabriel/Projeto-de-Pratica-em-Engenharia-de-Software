...
export default function CadastrarScreen() {
  ...
  const handleCadastro = async () => {
    ...
    const dto = {
      ...
      perfilUsuario: tipoPessoa === 'queroAdotar' ? 'ADOTANTE' : 'PROTETOR'
    ...
    <View style={styles.card}>
      ...
      <View style={styles.inputIconContainer}>
        ...
        <TouchableOpacity 
          ...
          onPress={() => setVisibilidades(prev => ({ ...prev, senha: !prev.senha }))}
      ...
      {showCrit && inputs.senha.length > 0 && (
        ...
        <View style={styles.critList}>
          ...
          {indicador.status.map(s => (
            ...
            <Text key={s.label} style={[styles.critText, { color: s.ok ? '#2AA5FF' : '#F25F5C' }]}>
      ...
      <View style={styles.inputIconContainer}>
        ...
        <TouchableOpacity 
          onPress={() => setVisibilidades(prev => ({ ...prev, senhaRep: !prev.senhaRep }))}
          style={styles.eyeIcon}
        >
...
const styles = StyleSheet.create({
  ...
  inputIconContainer: { // Corrigido - estava sendo referenciado como inputIcon no código
  ...
  passInput: {
    ...
    paddingVertical: 0, // Remove padding vertical para alinhar melhor
